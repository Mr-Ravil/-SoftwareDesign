...
public class StubServer implements AutoCloseable {
    ...
    public StubServer(int port, List<String> hosts, Duration duration) {
        ...
        for (String host : hosts) {
            stubServer.when(
                ...
                Gson g = new Gson();
                String responseBody = g.toJson(createResponse(host, query, top));
    ...
    public static Response createResponse(String host, String query, int top) {
        return new Response(host, IntStream.range(0, top).mapToObj(i -> host + "/query result: " + query + " top-" + i).toList());
    }
